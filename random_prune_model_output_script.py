...
seed = 42 # Set the random seed for reproducibility. we used 42 and 33
output_dir = "Qwen2.5-Math-1.5B-Instruct"
for value in values:
    base_path = os.path.join(f"Random pruned model{seed}", output_dir)
    model_id = os.path.join(base_path, f"{output_dir}-pruned-{value}p")
    for task_type in task_types:
        print(f"Generating outputs for {task_type} prompts...")
        load_model(model_id, task_type=task_type, output_dir=output_dir, half_precision=half_precision, value=value, model_type=f"random{seed}", chat_template=chat_template, batch_size=batch_size)
