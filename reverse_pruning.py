...
def physical_pruning(model, selectivity_scores, percentile=10.0):
    ...
    for layer_idx in range(num_layers):
        ...
        threshold = np.percentile(scores, 100.0 - percentile)
...
def helper(model_name, output_dir, percentile):
    ...
    # input_dir = f"C:/T2430447/Reverse Pruning/model activation/{output_dir}/selectivity_scores"
    input_dir = os.path.join(VOLUME_PATH, "selectivity_scores", output_dir)
    ...
    # save_pruned_model(pruned_model, tokenizer, save_dir=f"C:/T2430447/Pruned Model/{output_dir}/{output_dir}-pruned-{percentile}p")
    save_pruned_model(pruned_model, tokenizer, save_dir=os.path.join(VOLUME_PATH, "Reverse Pruned Model", output_dir, f"{output_dir}-pruned-{percentile}p"))



output_dir = "Qwen2.5-Coder-1.5B-Instruct"
os.makedirs(os.path.join(VOLUME_PATH, "Reverse Pruned Model", output_dir), exist_ok=True)
pruning_levels = [5.71, 10, 15.71, 20, 25.71, 30, 35.71]  # Qwen 1.5B
# pruning_levels = [25, 30.405, 35.135] #qwen 7b
