...
CHECKPOINT_FILE = os.path.join(VOLUME_PATH, f"checkpoint_pruning_{type[0]}.json")
...
for value in values:
    ...
    try:
        base_path = os.path.join(VOLUME_PATH, "Pruned Model", output_dir)
        ...
        for prompt_type in type:
            ...
            load_model(model_id, prompt_type, output_dir=output_dir, half_precision=half_precision, value=value, type="pruned", chat_template=chat_template, in_batch=True)
        ...
        # Don't mark as complete, so it will retry on next run
