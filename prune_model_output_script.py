...
for value in values:
    base_path = os.path.join("Pruned Model", output_dir)
    model_id = os.path.join(base_path, f"{output_dir}-pruned-{value}p")
    for task_type in task_types:
        print(f"Generating outputs for {task_type} prompts...")
        load_model(model_id, task_type=task_type, output_dir=output_dir, half_precision=half_precision, value=value, model_type="pruned", chat_template=chat_template, batch_size=batch_size)
